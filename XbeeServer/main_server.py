...
import logging
from XbeeSerialHost import XbeeSerialHost

XSH = XbeeSerialHost(pPort='', pBautrate=0)   # holds the XbeeSerialHost object, overwritten in the run function
DEV_MODE = True
...
def run(pHWport, pHWbautrate=9600, pSrvPort=8081, server_class=HTTPServer, handler_class=Srv):
    global XSH
    XSH = XbeeSerialHost(pPort=pHWport, pBautrate=pHWbautrate)
